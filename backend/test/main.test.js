...
describe('Thread post tests', () => {
  ...
  describe('PUT /thread/watch should', () => {
    ...
    it('successfully add a watch to your own post', async () => {
      ...
      const userInfo = await getTry(`/user?userId=${globals.ret2.userId}`, 200, {}, await globals.ret2.token);
      expect(userInfo.threadsWatching.includes(id)).toBe(true);
    });
    ...
    it('successfully add a watch to another persons public post', async () => {
      ...
      const userInfo = await getTry(`/user?userId=${globals.ret1.userId}`, 200, {}, await globals.ret1.token);
      expect(userInfo.threadsWatching.includes(id)).toBe(true);
    });
    ...
    it('successfully watch and unwatch something', async () => {
      ...
      const userInfo = await getTry(`/user?userId=${globals.ret1.userId}`, 200, {}, await globals.ret1.token);
      expect(userInfo.threadsWatching.includes(id)).toBe(false);
    });
    ...
    it('successfully have multiple people watch something', async () => {
      ...
      let userInfo = await getTry(`/user?userId=${globals.ret1.userId}`, 200, {}, await globals.ret1.token);
      expect(userInfo.threadsWatching.includes(id)).toBe(true);
      userInfo = await getTry(`/user?userId=${globals.ret2.userId}`, 200, {}, await globals.ret2.token);
      expect(userInfo.threadsWatching.includes(id)).toBe(true);
      userInfo = await getTry(`/user?userId=${globals.ret3.userId}`, 200, {}, await globals.ret3.token);
      expect(userInfo.threadsWatching.includes(id)).toBe(true);
    });
